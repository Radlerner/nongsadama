...
- Android에서 내 위치를 누르면 위치 권한을 요청하고, 대략적인 위치만 허용해도 가까운 지역을 찾는다.
- 한국어·영어·베트남어·크메르어·태국어·네팔어·몽골어·우즈베크어·러시아어 화면 문구를 제공한다.
- 기기 언어가 지원 목록에 있으면 처음 화면에 반영하고 언어 선택에서 추천 표시를 한다. 저장된 선택은 우선한다.
- 새 프로필에는 화면에 자동으로 적용된 기기 언어를 저장하되 `preferred_locale_explicit=false`로 두어 이후 국가 기본 언어를 따를 수 있게 한다.
- Kakao 지역 조회에는 약 11km 단위로 반올림한 위치만 전송한다.
