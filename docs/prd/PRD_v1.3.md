...
| 7 | 클라이언트 언어 플래그 | `I18nContext`: `nongsadama.locale.explicit` 저장, `setLocale(locale, explicit)`. `AuthContext`: 직접 고른 경우에만 프로필 생성 시 `preferred_locale`을 보내며, 로그인 전 선택도 세션 생성 뒤 기존 프로필에 `preferred_locale` + `explicit=true`로 UPDATE한다. 로그인 상태의 변경은 즉시 반영한다. `ProfileEdit`: 언어 select를 건드린 경우에만 언어를 저장, upsert → update/insert 분기. |
...
| 11 | 빌드·저장소 설정 | `build:release`가 Supabase와 STT 빌드 변수를 확인하고 누락 시 배포용 빌드를 중단한다. Kakao 지도 키는 공개 읽기 전용 `map_config`에서 실행 중에 읽으며 환경변수는 선택적 폴백이다. `deploy-pages.yml`이 저장소 Variable을 주입한다. `.env.example`에서 `VITE_STT_KEY` 삭제. 루트 `.gitignore`에 `supabase/.temp/`·에이전트 도구 폴더 추가, `android/.gitignore`의 키스토어 패턴(`*.jks`·`*.keystore`) 주석 해제(루트에는 이전부터 있음). `deploy-pages.yml`은 보조 GitHub Pages용이며 공식 웹은 Cloudflare 빌드다(§7). |
...
| `20260920000000_map_config.sql` | 테이블 추가 | 공개 Kakao Maps JavaScript 키 1개, anon/authenticated 읽기 전용 RLS | `drop table public.map_config;` |
