...
export type MarkerKind = 'point' | 'region'

const MARKER_RING_COLOR: Record<MarkerKind, string> = {
  point: '#1B4D3E', // Primary — 검수된 실좌표
  region: '#6C5CE7', // Accent — 좌표 미검수, 읍·면 중심 묶음
}

/** 이모지 마커 DOM(리플릿 divIcon과 동일한 시각 언어). onClick은 호출측에서 바인딩. */
export function emojiMarkerEl(emoji: string, count?: number, kind: MarkerKind = 'point'): HTMLElement {
  const ring = MARKER_RING_COLOR[kind]
  const el = document.createElement('div')
  el.style.cssText = `position:relative;width:38px;height:38px;background:#fff;border:2px solid ${ring};border-radius:9999px;display:flex;align-items:center;justify-content:center;font-size:19px;box-shadow:0 1px 3px rgba(0,0,0,.3);cursor:pointer;`
  el.textContent = emoji
  if (count && count > 1) {
    const badge = document.createElement('span')
    badge.style.cssText = `position:absolute;top:-6px;right:-6px;background:${ring};color:#fff;border-radius:9999px;font-size:11px;font-weight:700;min-width:18px;height:18px;line-height:18px;text-align:center;padding:0 3px;`
